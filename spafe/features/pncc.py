...
def asymmetric_lawpass_filtering(rectified_signal, lm_a=0.999,
                                 lm_b=0.5):
    floor_level = np.zeros_like(rectified_signal)
    floor_level[0] = 0.9 * rectified_signal[0]
    for m in range(floor_level.shape[0]):
        floor_level[m] = np.where(rectified_signal[m] >= floor_level[m
                                  - 1], lm_a * floor_level[m - 1] + (1
                                  - lm_a) * rectified_signal[m], lm_b
                                  * floor_level[m - 1] + (1 - lm_b)
                                  * rectified_signal[m])
...
def temporal_masking(rectified_signal, lam_t=0.85, myu_t=0.2):
    ...
    for m in range(1, rectified_signal.shape[0]):
        online_peak_power[m, :] = np.maximum(lam_t
                * online_peak_power[m - 1, :], rectified_signal[m, :])
        temporal_masked_signal[m, :] = np.where(rectified_signal[m, :]
                >= lam_t * online_peak_power[m - 1, :],
                rectified_signal[m, :], myu_t * online_peak_power[m
                - 1, :])
