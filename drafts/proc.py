...
def mcs(s):  # magnitude (actually power)
    return (np.abs(np.fft.ifft(np.log(np.abs(
        np.fft.fft(s))**2)))**2)[:len(s) // 2]
...
def normalize(file, noise_threshold=-80):
    ...
    while (i + 1378 < len(sound_wave)):
        if (i % 137800 == 0):
            print(
                str(i // 689) + '/' + str(len(sound_wave) // 689) +
                ' chunks normalized.')
        last_chunk_intensity = current_chunk_intensity
        current_chunk_intensity = intensity(sound_wave[i:i + 1378])
        for j in range(1378):
            if (0.5 * (last_chunk_intensity + current_chunk_intensity) <
                    maximum_intensity * noise_threshold):
                pass
            else:
                outsound[i + j] += float(
                    ((0.5 - 0.5 * math.cos(2 * math.pi * j / 1378)) *
                     float(sound_wave[i + j])) /
                    (last_chunk_intensity + current_chunk_intensity))
        i = i + 689
...
def _rawdata_to_array(data, channels, bytedepth):
    ...
    frame = np.array(struct.unpack('%dh' % N * channels,
                                   data)) / (2.0**(8 * bytedepth - 1))
